		...
		for (final Suit suit : Suit.values()) {
			...
			if ( !cardsForSuit.isEmpty()) {
				...
				if (previousCard.getValue() != ACE) {
					throw new IllegalArgumentException("First card in goal for suit " + suit + " must be an " + ACE + ", not a " + previousCard + ".");
				}
				while (iterator.hasNext()) {
					final Card currentCard = iterator.next();
					
					if (previousCard.getValue().getValue() + 1 != currentCard.getValue().getValue()) {
						throw new IllegalArgumentException("Cannot put " + currentCard + " on top of " + previousCard + " in goal for suit " + suit + ".");
					}
