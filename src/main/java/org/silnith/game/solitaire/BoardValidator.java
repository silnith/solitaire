...
public class BoardValidator implements Validator<Board> {
	...
	public void validate(final Board board) {
		...
		if (piles.size() != numberOfPiles) {
			throw new IllegalArgumentException("Board must have " + numberOfPiles + " piles, instead has " + piles.size());
		}
		...
		if (drawIndex > drawPile.size()) {
			throw new IllegalArgumentException("Draw index must not be larger than the draw pile size.  Draw pile has " + drawPile.size() + " cards, draw index is " + drawIndex);
		}
		// count the cards
		...
		if (numCards != deckSize) {
			throw new IllegalArgumentException("Must have " + deckSize + " cards on the board, instead has " + numCards + ".");
		}
	}
